"""Structural regression guard for the async migration of the icare extension
handlers.
...
coroutine (`async def`) and use `async with self.AsyncSession()`. This test:
  ...
  * imports each migrated handler module (catching import-time breakage such as
...
No external services are needed, so it runs anywhere the icare extensions are
...
# icare extension handler modules migrated to async (in-skyportal import paths).
HANDLER_MODULES = [
    "skyportal.handlers.api.classification",
    "skyportal.handlers.api.fink_photometry",
]
